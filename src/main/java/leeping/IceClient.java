...
public final class IceClient extends Thread implements Closeable{
   ...
   //代理类模块存储
    private  HashMap<Class,ObjectPrx> cls2PrxMap = new HashMap<>();
    /**
     * ice连接
     */
   ...
   //初始化
     ...
     * 获取代理 ,强制转换类型
     */
    public <T extends ObjectPrx> T getServicePrx(Class<T> clz) throws Exception{
    ...
    // 创建客户端服务代理
    private <T extends ObjectPrx> T createIceProxy(Class<T> clz) throws Exception {
       ...
       ObjectPrx iPrx= getIceCommunicator()
